...
//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	BYTE reg_select = reg + 2;
	alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg_select, 0, 0, 1);
	int ret_val = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &val, 0, 0, 0);
	if(ret_val < 0) {
		alt_printf("MAXred_wr Error");
	}
...
//return (data + nbytes);
	BYTE reg_select = reg + 2;
	alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg_select, 0, 0, 1);
	int ret_val = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes, data, 0, 0, 0);
	if(ret_val < 0) {
...
//return val
	BYTE val;
	int ret_val = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0);
	if(ret_val < 0) {
...
//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
//return (data + nbytes);
	int ret_val = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
	if(ret_val < 0) {
		alt_printf("MAXbtyes_rd Error");
	}
	return data + nbytes;
